...
from expenses.models import Accounts, Category
...
class CategoryForm(forms.Form):
    ...
    def clean(self):
        ...
        category = Category.objects.filter(budget__isnull=True,name=name).exists()
        if category is True:
            raise forms.ValidationError(u'Kategoria "%s" już istnieje.' % name)
        cleaned_data = super(CategoryForm, self).clean()
        #raise forms.ValidationError("Mój błąd")
        return cleaned_data


class TestFormWithoutRequiredClass(CategoryForm):
    required_css_class = ''
...
def render_template(text, **context_args):
    ...
    if not 'form' in context_args:
        context_args['form'] = CategoryForm()
