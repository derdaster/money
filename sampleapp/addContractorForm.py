...
from expenses.models import Accounts, Contractors
...
class ContractorForm(forms.Form):
    ...
    def clean(self):
        ...
        contractor = Contractors.objects.filter(budget__isnull=True,name=name).exists()
        if contractor is True:
            raise forms.ValidationError(u'Kontrahent "%s" już istnieje.' % name)
        cleaned_data = super(ContractorForm, self).clean()
        #raise forms.ValidationError("Mój błąd")
        return cleaned_data


class TestFormWithoutRequiredClass(ContractorForm):
    required_css_class = ''
...
def render_template(text, **context_args):
    ...
    if not 'form' in context_args:
        context_args['form'] = ContractorForm()
