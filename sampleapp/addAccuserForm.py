...
class AccuserForm(forms.Form):
    """
    Form with a variety of widgets to test bootstrap3 rendering.
    """
    def getUserList(self):
        budgetList=[(obj.id,obj.username) for obj in User.objects.filter(id__in=Budget.objects.filter())]
        return budgetList
    
    userList=getUserList(1)
    name = forms.ChoiceField(label=u'Konta',choices=userList,error_messages={'required':'Organizational unit is required field'})
    users2=forms.ChoiceField(label=u'Użytkownicy',choices=userList,error_messages={'required':'Organizational unit is required field'})  

    required_css_class = 'bootstrap3-req'

    def __init__(self, *args, **kwargs):
        user_id = kwargs.pop('user_id')
        super(AccuserForm, self).__init__(*args, **kwargs)
        self.fields['users2'].choices=([(obj.id, obj.username) for obj in User.objects.filter(id__in=Budget.objects.filter(id__in=Budget.objects.filter(members=User.objects.get(id=user_id.id))).values('members'))])
        self.fields['name'].choices=[(obj.id,obj.name) for obj in Accounts.objects.filter(members__in=User.objects.filter(id=user_id.id))]
    
        
    def clean(self):

        cleaned_data = super(AccuserForm, self).clean()
        # raise forms.ValidationError("Mój błąd")
        return cleaned_data


class TestFormWithoutRequiredClass(AccuserForm):
    required_css_class = ''
...
class SettingsTest(TestCase):        
    ...
    def test_bound_class(self):
        form = AccuserForm({'sender': 'sender'})
        res = render_template('{% bootstrap_form form %}', form=form)
